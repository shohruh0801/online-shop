<?php
require "db.php";
$admininform = mysqli_query($conn, "SELECT * FROM productscorusel");


if (isset($_FILES['img']) && isset($_POST['title']) && isset($_POST['header'])  ){
 $title = $_POST['title'];
 $header = $_POST['header'];
 $img_temp = $_FILES['img']['tmp_name'];
 $img_name = $_FILES['img']['name'];
 move_uploaded_file($img_temp, "./upload/". $img_name);
 $updt = mysqli_query($conn,"UPDATE productscorusel SET img='$img_name', header= '$header', title='$title'");
...
if (isset($_GET['id'])) {
$id = $_GET['id'];
$del = mysqli_query($conn, "DELETE FROM productscorusel WHERE id='$id'");
}
$serinform = "SELECT * FROM productscorusel";
